...
{    
	...
	private void validateConnectionUrl( String url, String jndiNameUrl )
	{
		if ( isBlank( url ) && isBlank( jndiNameUrl ) )
			throw new NullPointerException( "Wrong Url" );
	}
	...
	public IConnectionFactory getDriverConnectionFactory( String driverClass ) throws OdaException
	{
		...
		if ( driverInfo != null )
		{
			...
			if ( driverInfo instanceof IConfigurationElement )
			{
				...
				try
				{
					factory = (IConnectionFactory) ( (IConfigurationElement) driverInfo ).createExecutableExtension( "");
    ...
    {		
		...
		try
		{
			...
			if ( cachedDriversMap.get( driverClassName ) == null )
			{
				...
				// exception.
				if ( !canConnect )
					throw new JDBCException( "Error Url",
							null );
			}
			else
			{
		...
		if ( canConnect == false )
			throw new JDBCException( "Error Driver", null );
	...
	private Driver findDriver( String className, Collection<String> driverClassPath ) throws OdaException
	{
		...
		catch ( ClassNotFoundException e )
		{
				...
				if ( driverClass == null )
				{
					logger.warning( "Failed to load JDBC driver class: "
							+ className );
					throw new JDBCException( "Error cannot load Driver",
							null,
							className );
			}
		}
		...
		catch ( Exception e )
		{
			...
			throw new JDBCException( "Error cannot instantiate driver", null, className );
		}
		return driver;
	}
